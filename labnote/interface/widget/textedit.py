...
class CompleterTextEdit(QTextEdit):
    """ This QTextEdit subclass contains most functions required to handle a completer.

     It must be subclassed to implement the keyPressEvent function
     """
    ...
    def start_completer(self):
        ...
        completer.setModel(QStringListModel(self.completer_list))
...
class TagTextEdit(CompleterTextEdit):
    ...
    def __init__(self, completer_list):
        super(TagTextEdit, self).__init__(completer_list)
        self.textChanged.connect(self.set_base_format)
        self.format_completion.connect(self.make_format_completion)
        ...
        if is_tag:
            if not self.completer_status and self.textCursor().hasSelection():
                ...
                if not self.has_space(selection) and not self.has_word_separator(selection):
                    self.make_format_completion(self.textCursor().selectionStart(), self.textCursor().selectionEnd())
                    return
            ...
            if cursor.hasSelection():
                ...
                for position in range(cursor.selectionStart(), cursor.selectionEnd()):
                    ...
                    if cursor.charFormat().isAnchor():
                        ...
                        if prefix == 'tag':
                            tag_list.add(value)
                self.delete.emit(list(tag_list))
    ...
    def make_format_completion(self, *args):
        ...
        if len(args) == 0:
            ...
            self.stop_completer()
            self.completer_list.append(old_text)
            self.create.emit(old_text)
